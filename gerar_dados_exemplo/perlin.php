...
class Perlin
{
	...
	//funçao modificada pra retornar valores de 0 ~ 100 bem smooth
	function generatePerlinNoite2D($width)
	{
		$perlin = array();
		$frequency = 2 / $width;

		for ($x = 0; $x < $width; $x++) {
			$value = $this->smoothNoise($x * $frequency, $x * $frequency * 2, $x * $frequency / 2);
			$value = (($value +1) / 2) * 100;
			array_push($perlin, intval($value));
		}
